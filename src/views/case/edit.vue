        ...
        <div class="table-card-body">
          <el-form ref="caseEdit" :model="caseForm" label-width="80px">
            ...
            <el-form-item label="全文内容">
              <el-input v-model="caseForm.content" type="textarea" autosize/>
            </el-form-item>
            <el-form-item label="分析内容">
              <el-input v-model="caseForm.analyzeContent" type="textarea" autosize/>
            </el-form-item>
            ...
            <!-- <el-form-item label="类型">
              <el-select v-model="caseForm.type" placeholder="请选择案例类型">
...
import { getCaseDetails, editCaseDatails, getCaseThemeId, getCaseSubjectId } from '@/api/case'
...
export default {
  ...
  created() {
    ...
    this.caseDetails(this.$route.params.id)
    ...
    caseDetails(id) {
      getCaseDetails(id).then(res => {
        if (res.data.errorMsg === '操作成功') {
          this.caseForm.content = res.data.data.content
          this.caseForm.digest = res.data.data.digest
          this.caseForm.id = res.data.data.id
          this.caseForm.analyzeContent = res.data.data.analyzeContent
          this.caseForm.name = res.data.data.name
          this.caseForm.subjectId = res.data.data.subjectId
          this.caseForm.themeId = res.data.data.themeId
          this.caseForm.type = res.data.data.type
        }
      })
    },
    onCaseSubmit() {
      ...
      editCaseDatails(this.userId, this.caseForm).then(res => {
        if (res.data.errorMsg === '操作成功') {
          this.$route.go(-1)
        }
      })
